...
def get_model(**params):
    ...
    params['shift'] = params['shift'] * np.ones(3)
    ...
    def lorenz_63(x):
        return scipy.integrate.solve_ivp(lorenz63_f, [0.0, params['obs_gap']], x, method='RK45', t_eval=[params['obs_gap']]).y.T[0]

    # create a deterministic Markov chain
    prior = sm.Simulation(algorithm = lambda *args: params['shift'] + np.random.multivariate_normal(params['x0'], params['prior_cov']*id3))
    ...
    observation_noise = sm.Simulation(algorithm = lambda *args: np.random.multivariate_normal(zero2, params['obs_cov']*id2))
    conditional_pdf_o = lambda k, y, condition: scipy.stats.multivariate_normal.pdf(y, mean = func_o(0, condition, zero2), cov = params['obs_cov']*id2)
    ...
    mc = sm.DynamicModel(size=params['ev_time'], prior=prior, func=func_h, sigma=eps*id3, noise_sim=process_noise, conditional_pdf=conditional_pdf_h)
    om = sm.MeasurementModel(size=params['ev_time'], func=func_o, sigma=params['obs_cov']*id2, noise_sim=observation_noise, conditional_pdf=conditional_pdf_o)
