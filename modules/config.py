...
class ConfigCollector:
	...
	"""
	def __init__(self, folder, config_dict = {}, overwrite=True):
		...
		self.res_path = self.folder#os.path.join(folder, self.expr_name)
	...
	def write(self, mode='json'):
		...
		if mode == 'txt':
			with open(self.folder + '/config.txt', 'w') as f:
				for k in sorted(self.config):
					f.write('{}: {}\n'.format(k, self.config[k]))
		elif mode == 'json':
			with open(self.folder + '/config.json', 'w') as f:
				json.dump(self.config, f, indent=2, cls=ut.NumpyEncoder)
