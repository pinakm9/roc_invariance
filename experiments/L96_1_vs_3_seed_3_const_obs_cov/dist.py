...
_experiment_params['tag'] = ['prior_3_gap_{:.2f}'.format(gap) for gap in model_params['obs_gap']] 
...
for i, folder_1 in enumerate(folder_list_1):
    ...
    df = pd.read_csv(file)
    steps[i] = find_stability_sma(df['sinkhorn_div'].to_numpy(), tail=250) + 1
...
ax.scatter(gaps, steps, label='steps')
ax.plot(gaps, gaps*steps, label='real time' )
